...
import { acceptConnectionRequest, RespondConnectionRequestError } from '@/lib/connection-requests'
...
/**
 * POST /api/app/connections/requests/[fromUserId]/accept
 * Accept a pending connection request from that user.
 */
export async function POST(
  ...
  try {
    await acceptConnectionRequest(uid, from)
    return NextResponse.json({ ok: true })
  } catch (err) {
    if (err instanceof RespondConnectionRequestError) {
      return NextResponse.json({ error: err.message }, { status: err.status })
    }
    console.error('[app connections requests accept POST]', err)
