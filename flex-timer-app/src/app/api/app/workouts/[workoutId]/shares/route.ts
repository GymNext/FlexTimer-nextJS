...
export async function GET(_request: NextRequest, { params }: { params: RouteParams }) {
  ...
  try {
    const w = await getWorkoutById(uid, workoutId)
    if (!w || w.deletedAt) {
      return NextResponse.json({ error: 'Workout not found' }, { status: 404 })
    }

    const [groupRows, userRows] = await Promise.all([
      listResourceGroupShares(uid, 'workout', workoutId),
      listResourceUserShares(uid, 'workout', workoutId),
    ])
    ...
    const destinationCount = await countResourceShareDestinations(uid, 'workout', workoutId)
    ...
    console.error('[workout shares GET]', err)
    return NextResponse.json(
...
export async function POST(request: NextRequest, { params }: { params: RouteParams }) {
  ...
  try {
    ...
    if (target === 'group') {
      ...
      await shareResourceWithGroup(uid, 'workout', workoutId, gid, body.comment ?? null)
    ...
    await shareResourceWithUser(uid, 'workout', workoutId, peer, body.comment ?? null)
    return NextResponse.json({ ok: true })
  } catch (err) {
    ...
    console.error('[workout shares POST]', err)
    return NextResponse.json(
      { error: err instanceof Error ? err.message : 'Failed to share workout' },
      { status: 500 }
    )
  }
}
...
export async function DELETE(request: NextRequest, { params }: { params: RouteParams }) {
  ...
  try {
    ...
    if (target === 'group') {
      ...
      await stopSharingResourceWithGroup(uid, 'workout', workoutId, groupId, feedHint)
    ...
    await stopSharingResourceWithUser(uid, 'workout', workoutId, peerUserId, feedHint)
    return new NextResponse(null, { status: 204 })
  } catch (err) {
    const e = err as Error & { status?: number }
    const status = typeof e.status === 'number' ? e.status : 500
    ...
    console.error('[workout shares DELETE]', err)
    return NextResponse.json(
